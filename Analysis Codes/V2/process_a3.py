...
USAGE = f"Usage: python3 process_{FEATURE_ID}.py <A3_hits_file> <source_js_file>"


def get_label(hits_path: str) -> str:
    """
    Strip leading 'A3_' from basename to get a per-file label.

    Example:
        A3_home--...--bundle_flow.js -> home--...--bundle_flow.js
    """
...
def main():
    ...
    # ----------------------------------------------------------------
    # Load source (bundle_flow.js or segmented file)
    # ----------------------------------------------------------------
    ...
    with open(counts_out, "w", encoding="utf-8") as out:
        ...
        out.write(f"{FEATURE_ID}_CONFIG_OR_URL: {len(buckets['CONFIG_OR_URL'])}\n")
        out.write(f"{FEATURE_ID}_LOG_OR_MESSAGE: {len(buckets['LOG_OR_MESSAGE'])}\n")
    ...
    with open(detail_out, "w", encoding="utf-8") as out:
        ...
        out.write(f"A3_CONFIG_OR_URL: {len(buckets['CONFIG_OR_URL'])}\n")
        out.write(f"A3_LOG_OR_MESSAGE: {len(buckets['LOG_OR_MESSAGE'])}\n")
