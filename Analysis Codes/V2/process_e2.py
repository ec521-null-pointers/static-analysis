...
USAGE = f"Usage: python3 process_{FEATURE_ID}.py <E2_hits_file>"
...
def main():
    ...
    # outputs
    hits_out = f"{FEATURE_ID}_hits_{label}"
    details_out = f"{FEATURE_ID}_2_{label}"

    entries = parse_e2(e2_path)
    summary = summarize(entries)

    # ---- counts file for scoring ----
    with open(hits_out, "w", encoding="utf-8") as out:
        if summary is None:
    ...
    print(f"[+] {FEATURE_ID} counts written to {hits_out}")
