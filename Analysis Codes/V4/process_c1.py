...
process_C1.py
Usage:
    python3 process_C1.py <C1_hits_file> <analysis_root>

Reads:
    - Raw URL hits file (from ripgrep or similar), each line containing 0+ http(s) URLs.

Writes:
    - C1_score_<label>   (counts vector)
    - C1_detail_<label>  (classified summary with per-host details and aggregate stats)
"""
...
def get_label(hits_path: str) -> str:
    ...
    Example:
        C1_extraction_foo.js.txt -> extraction_foo.js.txt
    """
...
def resolve_output_paths(analysis_root: str, label: str):
    ...
    Outputs:
        <analysis_root>/Scores/C1_score_<label>
        <analysis_root>/Details/C1_detail_<label>
...
def main():
    if len(sys.argv) < 3:
        print(f"Usage: python3 process_{FEATURE_ID}.py <C1_hits_file> <analysis_root>")
        sys.exit(1)
    ...
    write_summary(hosts, stats, detail_out, hits_path, label)
