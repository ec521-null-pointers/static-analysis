...
Manual logistic regression with a quadratic feature map:
...
def main():
    ...
    # Log-scaled logits for plotting / CSV  (best for quadratic)
    norm_scores = np.sign(logit_scores) * np.log1p(np.abs(logit_scores))
    ...
    # Sort by normalized score for Figure 1
    ...
    plt.ylim(-0.1, 1.1)
    plt.title("Quadratic Logistic Regression: probability vs normalized score")
    plt.legend(loc="best")
    plt.tight_layout()
