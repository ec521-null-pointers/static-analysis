...
Logistic regression with RBF kernel approximated via random Fourier features:

    φ(x) = sqrt(2/D) * cos(W x + b)

Same PU-learning flavor:
  - label 1 = known malicious, high weight.
  - label 0 = assumed-good, lower weight.
  - threshold chosen by allowed fraction of 'good' flagged.

Outputs analogous to linear/quadratic, plus saving RFF W,b.
"""
...
def main():
    ...
    plt.ylim(-0.1, 1.1)
    plt.title("RBF Logistic Regression (RFF): probability vs signed-log score")
    plt.legend(loc="best")
    plt.tight_layout()
